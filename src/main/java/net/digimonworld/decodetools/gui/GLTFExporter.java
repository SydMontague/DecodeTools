...
public class GLTFExporter {
	    ...
	    { 
    	 ...
    	 GlTF gltf = GltfCreatorV2.create(gltfModel);
          ...
          // Create buffer views
             ...
             normalBufferView.setByteOffset(posBufferView.getByteOffset() + posBufferView.getByteLength()); // start where positions left off
             ...
             indexBufferView.setByteOffset(normalBufferView.getByteOffset() + normalBufferView.getByteLength()); // start where normals left off
 ...
 private static List<Float> vertexAttribToList(List<XTVOVertex> vertices, XTVORegisterType type) {
    return vertices.stream().map(a -> a.getParameter(type)).flatMap(a -> a.getValue().stream().map(b -> a.getKey().getValue(b))).collect(Collectors.toList());
}
}