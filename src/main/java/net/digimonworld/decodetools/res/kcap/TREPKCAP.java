...
import net.digimonworld.decodetools.res.payload.TREPPayload;

public class TREPKCAP extends AbstractKCAP {
    
    private List<TREPPayload> entries = new ArrayList<>();
    
    public TREPKCAP(AbstractKCAP parent, Access source, int dataStart, KCAPInformation info) {
        super(parent, info.flags);
        
        // make sure it's actually a TREP
        if (source.readInteger() != KCAPType.TREP.getMagicValue())
            throw new IllegalArgumentException("Tried to instanciate TREP KCAP, but didn't find a TREP header.");
        ...
        for (KCAPPointer p : pointer) {
            ...
            entries.add(new TREPPayload(source, dataStart, this, p.getSize(), null));
        }
        
        // make sure we're at the end of the KCAP
        long expectedEnd = info.startAddress + info.size;
        if (source.getPosition() != expectedEnd)
            Main.LOGGER.warning(() -> "Final position for TREP KCAP does not match the header. Current: " + source.getPosition() + " Expected: " + expectedEnd);
    }
