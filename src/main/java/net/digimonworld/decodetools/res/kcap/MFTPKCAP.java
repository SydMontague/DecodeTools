...
import net.digimonworld.decodetools.res.payload.MFTPPayload;

public class MFTPKCAP extends AbstractKCAP {
    
    private List<MFTPPayload> entries = new ArrayList<>();
    
    public MFTPKCAP(AbstractKCAP parent, Access source, int dataStart, KCAPInformation info) {
        super(parent, info.flags);
        
        // make sure it's actually a MFTP
        if (source.readInteger() != KCAPType.MFTP.getMagicValue())
            throw new IllegalArgumentException("Tried to instanciate MFTP KCAP, but didn't find a MFTP header.");
        ...
        for (KCAPPointer p : pointer) {
            ...
            entries.add(new MFTPPayload(source, dataStart, this, p.getSize(), null));
        }
        
        // make sure we're at the end of the KCAP
        long expectedEnd = info.startAddress + info.size;
        if (source.getPosition() != expectedEnd)
            Main.LOGGER.warning(() -> "Final position for MFTP KCAP does not match the header. Current: " + source.getPosition() + " Expected: " + expectedEnd);
    }
