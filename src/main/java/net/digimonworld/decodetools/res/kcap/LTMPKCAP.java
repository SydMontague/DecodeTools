...
import net.digimonworld.decodetools.res.payload.LTMPPayload;

public class LTMPKCAP extends AbstractKCAP {
    
    private List<LTMPPayload> entries = new ArrayList<>();
    
    public LTMPKCAP(AbstractKCAP parent, Access source, int dataStart, KCAPInformation info) {
        super(parent, info.flags);
        
        // make sure it's actually a LTMP
        if (source.readInteger() != KCAPType.LTMP.getMagicValue())
            throw new IllegalArgumentException("Tried to instanciate LTMP KCAP, but didn't find a LTMP header.");
        ...
        for (KCAPPointer p : pointer) {
            ...
            entries.add(new LTMPPayload(source, dataStart, this, p.getSize(), null));
        }
        
        // make sure we're at the end of the KCAP
        long expectedEnd = info.startAddress + info.size;
        if (source.getPosition() != expectedEnd)
            Main.LOGGER.warning(() -> "Final position for LTMP KCAP does not match the header. Current: " + source.getPosition() + " Expected: " + expectedEnd);
    }
