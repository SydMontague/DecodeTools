...
import net.digimonworld.decodetools.res.payload.TMEPPayload;

public class TMEPKCAP extends AbstractKCAP {
    
    private List<TMEPPayload> entries = new ArrayList<>();
    
    public TMEPKCAP(AbstractKCAP parent, Access source, int dataStart, KCAPInformation info) {
        super(parent, info.flags);
        
        // make sure it's actually a TMEP
        if (source.readInteger() != KCAPType.TMEP.getMagicValue())
            throw new IllegalArgumentException("Tried to instanciate TMEP KCAP, but didn't find a TMEP header.");
        ...
        for (KCAPPointer p : pointer) {
            ...
            entries.add(new TMEPPayload(source, dataStart, this, p.getSize(), null));
        }
        
        // make sure we're at the end of the KCAP
        long expectedEnd = info.startAddress + info.size;
        if (source.getPosition() != expectedEnd)
            Main.LOGGER.warning(() -> "Final position for TMEP KCAP does not match the header. Current: " + source.getPosition() + " Expected: " + expectedEnd);
    }
