...
import net.digimonworld.decodetools.res.payload.LDMPPayload;

public class LDMPKCAP extends AbstractKCAP {
    
    private List<LDMPPayload> entries = new ArrayList<>();
    
    public LDMPKCAP(AbstractKCAP parent, Access source, int dataStart, KCAPInformation info) {
        super(parent, info.flags);
        
        // make sure it's actually a LDMP
        if (source.readInteger() != KCAPType.LDMP.getMagicValue())
            throw new IllegalArgumentException("Tried to instanciate LDMP KCAP, but didn't find a LDMP header.");
        ...
        for (KCAPPointer p : pointer) {
            ...
            entries.add(new LDMPPayload(source, dataStart, this, p.getSize(), null));
        }
        
        // make sure we're at the end of the KCAP
        long expectedEnd = info.startAddress + info.size;
        if (source.getPosition() != expectedEnd)
            Main.LOGGER.warning(() -> "Final position for LDMP KCAP does not match the header. Current: " + source.getPosition() + " Expected: " + expectedEnd);
    }
